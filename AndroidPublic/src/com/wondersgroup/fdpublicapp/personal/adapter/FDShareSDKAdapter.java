 ...
 *后执行关注的方法。
 */
public class FDShareSDKAdapter extends AuthorizeAdapter implements OnClickListener, PlatformActionListener {
	...
	public void onCreate() {
		...
		llTitle.getTvTitle().setText("全程食安用户登录");
		llTitle.setBackgroundResource(R.drawable.main_header_bg);
...
//			ctvFollow.setText(R.string.sm_item_fl_tc);
	...
	public void onError(Platform plat, int action, Throwable throwable) {
		if (action == Platform.ACTION_AUTHORIZING) {
			...
			if (backListener != null) {
				backListener.onError(plat, action, throwable);
			}
		}else {
			// 关注时发生错误
			plat.setPlatformActionListener(backListener);
	...
	public void onComplete(Platform plat, int action,HashMap<String, Object> res) {
		if (action == Platform.ACTION_FOLLOWING_USER) {
			...
			// 授权成功，执行关注
			String account = "授权成功";    // MainAdapter.SDK_SINAWEIBO_UID;
